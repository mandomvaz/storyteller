...
The system SHALL generate a 5-emojis badge for a story using a `KernelFunction` created via SK's `CreateFunctionFromPrompt`. The function SHALL receive the story title and full body text as arguments and return exactly 5 emojis.
...
- **AND** the system SHALL return the result string (5 emojis)
...
- **AND** SHALL use a built-in default prompt: "Genera exactamente 5 emojis que representen el tono de esta historia. Devuelve solo los 5 emojis, nada más."
