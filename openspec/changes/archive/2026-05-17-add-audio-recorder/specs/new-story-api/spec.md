...
The system SHALL provide a `POST /api/stories/new` endpoint that accepts audio file uploads and returns a story identifier.

#### Scenario: Successful audio upload
- **WHEN** a POST request is sent to `/api/stories/new` with a multipart form containing an audio file in the `audio` field
- **THEN** the endpoint SHALL return HTTP 200 with a JSON body containing `{ "storyId": "<guid>" }`
