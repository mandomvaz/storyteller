...
The `POST /api/stories/new` endpoint SHALL accept audio input, transcribe it via whisper, synthesize the transcript via XTTS, and return the resulting audio as a binary `audio/wav` response.

#### Scenario: Successful audio pipeline
- **WHEN** a valid `multipart/form-data` request with an `audio` field is sent
- **THEN** the endpoint SHALL return HTTP 200 with `Content-Type: audio/wav` and the synthesized audio as the body
